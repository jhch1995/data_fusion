...
public:
	...
	/// 提供一个图像
	void ProcessNewImage(const cv::Mat& img, int frame_id);
	...
	// 从旋转矩阵R计算姿态角Euler
    void CalculateEulerAngleFromRotation(const cv::Mat& R, double euler_angle[3]);
    
    // 获取姿态角Euler
    void GetCurrentDiffEuler(double euler_angle[3]);
